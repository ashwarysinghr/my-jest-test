import React from 'react';
import { mount, shallow } from 'enzyme';
import { Car } from './Car';
import Engine from './Engine';
import toJson from 'enzyme-to-json';

describe('<Car />', () => {
  let wrapper;
  let handleCarStart;
  beforeAll(() => { // declaring function and Car prototype
    handleCarStart = jest.fn();
    wrapper = shallow(<Car color='red' handleCarStart={handleCarStart} />);
  });
  it('matches snapshot', () => {
    expect(toJson(wrapper)).toMatchSnapshot();
  });

  // check dynamic data renders correctly
  it('renders the heading', () => {
    const heading = wrapper.find('#car--heading');
    expect(heading).toHaveLength(1);
    expect(heading.text()).toEqual('I am a red car');
  });

  // check child components are called correctly
  it('has a diesel engine', () => {
    const engine = wrapper.find(Engine);
    expect(engine.prop('type')).toEqual('diesel');
  });
  
  // check actions
  it('calls handleCarStart on button click', function () {
    const button = wrapper.find('#car--start');
    expect(button).toHaveLength(1);
    button.simulate('click');
    expect(handleCarStart).toHaveBeenCalledWith({ startTime: 223 });
  });
});
